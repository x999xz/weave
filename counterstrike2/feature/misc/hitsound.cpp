...
#include "hitsound.h"
#include <algorithm>
#include "../event/sounds.h"

void hitsound::WavHeader_t::ParseWavHeader(unsigned char* wavData) {
...
bool hitsound::AdjustHitsoundVolume(hitsound::WavHeader_t& header, float volume)
{
...
void hitsound::run()
{
	if (m_should_parse_hitsound) {
		...
		hitsound::WavHeader_t header;
		header.ParseWavHeader(parsed_hitsound);
		g_hitsounds->AdjustHitsoundVolume(header, 1.f);

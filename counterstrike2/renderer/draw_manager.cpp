...
inline float inv_length(const position& lhs, const float fail_value)
{
...
inline void generate_circle_metadata(size_t& start_idx, size_t& point_count, size_t& skip_count, const pos_type radius, pos_type degrees, pos_type start_degree)
{
	degrees = std::clamp(std::fabs(degrees), 0.f, 360.f);
...
inline void generate_circle_points(position* point_buf, const size_t start_idx, const size_t point_count, const size_t skip_count, const pos_type radius, const position& center)
{
	...
	const position& bot_right)
{
	...
	cur_font->render_text(this, cur_font->original_font_size, top_left, col, clip_rect, text, text + strlen(text));
	...
	const position& bot_right)
{
	...
	if (outline)
	{
		const auto col_out = color{ 0, 0, 0, col.a() };
		auto copy_clip = clip_rect;
		copy_clip.xy = top_left;
		copy_clip.x -= 1.f;
		copy_clip.z -= 1.f;
	...
	const int rasterizer_flags) const
{
	...
	const font_wchar* ranges = fonts->glyph_ranges_default();
	...
	if (range & GLYPH_RANGE_CYRILLIC)
		ranges = fonts->glyph_ranges_cyrillic();
	...
	const uint8_t flags)
{
	...
	buf->push_clip_rect(top_left, bot_right - position(1, 1));

	if (flags & ROUND_RECT_TL)
		buf->circle_filled(corner_tl, radius, col_top_left, col_top_left, 64, 45, 270);
	else
		buf->rectangle_filled(top_left, corner_tl, col_top_left);

	if (flags & ROUND_RECT_TR)
		buf->circle_filled(corner_tr, radius, col_top_right, col_top_right, 64, 45, 0);
	else
		buf->rectangle_filled({ corner_tr.x, top_left.y }, { bot_right.x, corner_tr.y }, col_top_right);

	if (flags & ROUND_RECT_BR)
		buf->circle_filled(corner_br, radius, col_bot_right, col_bot_right, 64, 45, 90);
	else
		buf->rectangle_filled(corner_br, bot_right, col_bot_right);

	if (flags & ROUND_RECT_BL)
		buf->circle_filled(corner_bl, radius, col_bot_left, col_bot_left, 64, 45, 180);
	else
